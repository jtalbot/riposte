...
#define NULLARY(op,...) case IROpCode::op: break;
		...
		case IROpCode::loadna: out << "$" << node.in.p << "[" << node.constant.i << "]\t"; break;
...
IRef Trace::EmitUnary(IROpCode::Enum op, Type::Enum type, IRef a, int64_t data) {
	...
	if(	(op == IROpCode::sum || op == IROpCode::prod || 
		...
		n.outShape = (IRNode::Shape) { nodes[a].outShape.levels, -1, 1, -1, true, -1 };
...
IRef Trace::EmitGenerator(IROpCode::Enum op, Type::Enum type, int64_t length, int64_t a, int64_t b) {
	...
	n.op = op;
	n.type = type;
	n.shape = (IRNode::Shape) { length, -1, 1, -1, false, -1 };
	n.outShape = n.shape;
	n.sequence.ia = a;
	n.sequence.ib = b;
	nodes.push_back(n);
...
IRef Trace::EmitLoad(Value const& v, int64_t length, int64_t offset) {
	...
	n.op = IROpCode::load;
	n.type = v.type;
	n.shape = (IRNode::Shape) { length, -1, 1, -1, false, EmitLoadNA(v, length, offset) };
	n.outShape = n.shape;
	n.constant.i = offset;
	n.in = v;
	nodes.push_back(n);
...
IRef Trace::EmitLoadNA(Value const& v, int64_t length, int64_t offset) {
	IRNode n;
	n.arity = IRNode::NULLARY;
	n.group = IRNode::GENERATOR;
	n.op = IROpCode::loadna;
	n.type = v.type;
	n.shape = (IRNode::Shape) { length, -1, 1, -1, false, -1 };
	n.outShape = n.shape;
	n.constant.i = offset;
...
IRef Trace::EmitMergeNA(IRef a, IRef b) {
	if(nodes[a].outShape.na == nodes[b].outShape.na) return nodes[a].outShape.na;
	if(nodes[a].outShape.na == -1) return nodes[b].outShape.na;
	if(nodes[b].outShape.na == -1) return nodes[a].outShape.na;
	IRNode n;
	n.arity = IRNode::BINARY;
	n.group = IRNode::MAP;
	n.op = IROpCode::lor;
	n.type = Type::Logical;
	n.binary.a = nodes[a].outShape.na;
	n.binary.b = nodes[b].outShape.na;
	n.shape = (IRNode::Shape) { nodes[a].outShape.length, -1, 1, -1, false, -1 };
	n.outShape = n.shape;
	nodes.push_back(n);
	return nodes.size()-1;
}
...
IRef Trace::EmitSLoad(Value const& v) {
	...
	n.op = IROpCode::sload;
	n.type = v.type;
	n.shape = (IRNode::Shape) { Size, -1, 1, -1, false, -1 };
	n.outShape = (IRNode::Shape) { v.length, -1, 1, -1, true, -1 };
	n.in = v;
	nodes.push_back(n);
	return nodes.size()-1;
}
...
IRef Trace::EmitSStore(IRef ref, int64_t index, IRef v) {
	...
	n.shape = (IRNode::Shape) { Size, -1, 1, -1, false, -1 };
	n.outShape = (IRNode::Shape) { std::max(nodes[ref].outShape.length, index), -1, 1, -1, true, -1 };
	n.binary.a = ref;
	n.binary.b = v;
	n.binary.data = index;
	nodes.push_back(n);
