...
expr(A) ::= FUNCTION(B) optnl LPAREN optnl formallist(C) optnl RPAREN optnl statement(D).  { Value p; Object::InitWithNames(p, C->values(), C->names(true)); A = CreateCall(List::c(B, p, D, Character::c(parser->popSource()))); }
