...
Value asnull(State& state, List const& args) {
	Value from = force(state, args[0]);
	return As<Null>(state, from);
}

Value aslogical(State& state, List const& args) {
	Value from = force(state, args[0]);
	return As<Logical>(state, from);
}

Value asinteger(State& state, List const& args) {
	Value from = force(state, args[0]);
	return As<Integer>(state, from);
}

Value asdouble(State& state, List const& args) {
	Value from = force(state, args[0]);
	return As<Double>(state, from);
}

Value ascomplex(State& state, List const& args) {
	Value from = force(state, args[0]);
	return As<Complex>(state, from);
}

Value ascharacter(State& state, List const& args) {
	Value from = force(state, args[0]);
	return As<Character>(state, from);
}

Value aslist(State& state, List const& args) {
	Value from = force(state, args[0]);
...
/*
Value isnull(State& state, List const& args) {
	Value from = force(state, args[0]);
	state.registers[0] = Logical::c(from.isNull());
	return 1;
}

Value islogical(State& state, List const& args) {
	Value from = force(state, args[0]);
	state.registers[0] = Logical::c(from.isLogical());
	return 1;
}

Value isinteger(State& state, List const& args) {
	Value from = force(state, args[0]);
	state.registers[0] = Logical::c(from.isInteger());
	return 1;
}

Value isdouble(State& state, List const& args) {
	Value from = force(state, args[0]);
	state.registers[0] = Logical::c(from.isDouble());
	return 1;
}

Value iscomplex(State& state, List const& args) {
	Value from = force(state, args[0]);
	state.registers[0] = Logical::c(from.isComplex());
	return 1;
}

Value ischaracter(State& state, List const& args) {
	Value from = force(state, args[0]);
	state.registers[0] = Logical::c(from.isCharacter());
	return 1;
}

Value islist(State& state, List const& args) {
	Value from = force(state, args[0]);
