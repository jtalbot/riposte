...
extern "C"
void fileinfo_map(Thread& thread,
    ...
    String& f)
{
    ...
    if( stat(f, &t) != -1) {
        ...
        if(p != NULL)
            uname = thread.internStr(p->pw_name);
        else
            grname = Character::NAelement;

        group const* g = getgrgid(t.st_gid);        
        if(g != NULL)
            grname = thread.internStr(g->gr_name);
        else
            grname = Character::NAelement;
    }
    else {
...
extern "C"
Value sysglob(Thread& thread, Value const* args)
{
    ...
    for(size_t i = 0; i < gl.gl_pathc; ++i) {
        r[i] = thread.internStr(gl.gl_pathv[i]);
    }
