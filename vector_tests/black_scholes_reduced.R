...
blackScholes <- function(N_BLACK_SCHOLES_ROUNDS,N_OPTIONS) {
	...
	for(i in 1:N_BLACK_SCHOLES_ROUNDS) {
		delta <- v * -(TT)
		d1 <- (-(S/X)/log10 + (r + v * v * .5) * TT) / delta
		d2 <- d1 - delta
		acc <- acc + sum(S * CND(d1) - X * -(-r * TT) * CND(d2))
	}
...
time_many_sizes("black_scholes_reduced",1,blackScholesInit,blackScholes, baseline)
